...
class OpenAI:
    def __init__(self, client):
        self.api_key = client.access_key or os.getenv("OPENAI_API_KEY")
        if not self.api_key:
            raise ValueError("API key must be provided or set in OPENAI_API_KEY environment variable")
    ...
    def _create_completion(
        ...
        url = f"{self.base_url}/api/model/openai"
        headers = {
        ...
        for key, value in optional_params.items():
            if value is not None:
                payload[key] = value
        print(headers)
        response_data = self.base_client.make_request("POST", url, headers=headers, json=payload)
        return OpenAIResponse(**response_data)
